...
if __name__=="__main__":
    ...
    outerSpherePoints = outerSpherePointsBB.blockXYZVals 

    # generate the XYZVals packed in the inner sphere
    innerSpherePointsBB = SphereBBG.generateBuildingBlock(numPolymersPerSphere, SphereRadius, ThetaMin, ThetaMax, PhiMin, PhiMax, UnimerBaseRadius)
    innerSpherePointsBB.transformBBToLabFrame(np.array([0.0, 0.0, 1.0]), centerPos, 0.0)
    innerSpherePointsBB.exportBBK("innerSphereBasePoints")
    innerSpherePoints = innerSpherePointsBB.blockXYZVals 


    # find the radial director vectors at each point
    outerDirectorsHat = [ director/np.linalg.norm(director) for director in outerSpherePoints]
    innerDirectorsHat = [ -director/np.linalg.norm(director) for director in outerSpherePoints]

    # generate unique polymer strands
    for i in range(numPolymersPerSphere):
        print(i, "unimers out of: ", numPolymersPerSphere)
        if i==0:
            outerUnimerStrands=[GeneralBlockPolymer(polymerBrushDict)]
        else:
            outerUnimerStrands.append(GeneralBlockPolymer(polymerBrushDict))
