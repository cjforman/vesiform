...
class NPack(keyProc):
    ''' This class provides the core algorithm and checking routine to generate a list of randomly 
    chosen numpy vectors that satisfy the applied constraints. Some example classes are provided showing 
    how to use the class - 3DSpacePack, Flat, Cylinder, Sphere, Ellipsoids and Chain.
    The algorithm is parameter driven by textfile and extends the keyProc class which automatically parses 
    and generates dictionaries of parameters based on the information in a text file.
    
    Parameters that must be defined in the params file are:

    numPoints:  numPoints in a list 
    minDist:  minimum distance between points in xyz space.

    maxNumPrunes: Maximum number of times a prune can occur since the current maximum length was last incremented
    maxLivesPerNode: the max number of times a point can be the penultimate point in the list before it is deleted.
    maxLivesPerNewNode: Number of times we try to find a new point that satisfies the conditions before being deleted. 
    maxNumReRoots: max number of times we can reselect the starting points before completely bailing.  
    reRootAfterTooManyPrunes: Flag to indicate whether or not we should trigger a reroot if we exhaust maxNumPrunes. 

    verbose 0, 1, 2 different levels of verbosity output to the console to determine what is happening. 
    ...
    def initialiseParameters(self):
        ...
        self.numPoints = self.getParam('numPoints') 
    ...
    def pickRandomPointInDefinedSpace(self):
        ...
        # default is a point in cube between (-1, 1) in x, y and z.
        return np.array([rnd.uniform(-1.0, 1.0), rnd.uniform(-1.0, 1.0), rnd.uniform(-1.0, 1.0)]) 
         
        
    def checkPointInBounds(self, newPoint):
        ''' Returns true if the specified point is in bounds and false if it is out of bounds.
        Checks the position against the constraints defined in this function. The position 
        vector is a numpy 3-array defined by the pickRandomPointInDefinedSpace and the check
        function assumes that the User understands the format of the points in these tests. '''
        # TO DO: Over load this function to ensure that a picked if is within the defined limits for the space 
        inBounds = True # assume point is in bounds
        return inBounds
